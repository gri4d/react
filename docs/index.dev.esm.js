...
(() => {
  ...
  var x = class {
    ...
    findFirstVisibleIndex(e) {
      let n = 0, t = this.visibleRanges.length - 1;
      for (; n <= t; ) {
        let i = Math.floor((n + t) / 2);
        this.visibleRanges[i].bottom < e ? n = i + 1 : t = i - 1;
      }
      return n;
    }
    findLastVisibleIndex(e) {
      let n = 0, t = this.visibleRanges.length - 1;
      for (; n < t; ) {
        let i = Math.ceil((n + t) / 2);
        this.visibleRanges[i].top < e ? n = i : t = i - 1;
      }
      return n;
    }
  };
  ...
  var M = class {
    ...
    reset(e) {
      ...
      this.gridSizingElement.style.height = `${Math.min(G, this.gridHeight)}px`, Object.assign(this.itemsViewportElement.style, { width: "100%", overflow: "hidden", position: "sticky", height: "100vh", top: s + "px" }), clearTimeout(this.resizeTimeout), window.removeEventListener("resize", this.onResize), window.addEventListener("resize", this.onResize);
      let g = this.getViewportElement();
      g.removeEventListener("scroll", this.onScroll), g.addEventListener("scroll", this.onScroll);
    ...
    destroy() {
      window.removeEventListener("resize", this.onResize), this.getViewportElement()?.removeEventListener("scroll", this.onScroll), this.wrapperElement?.remove();
    }
    ticking = false;
    onScroll = () => {
      this.ticking || (requestAnimationFrame(() => {
        this.update(), this.ticking = false;
      }), this.ticking = true);
    };
    resizeTimeout = -1;
    onResize = () => {
    ...
    getViewportType() {
      ...
      return e instanceof HTMLElement ? 1 : e && typeof e.height == "number" ? 2 : 0;
    }
    getViewportElement() {
      let e = this.getViewportType();
      return e === 1 ? this.options.viewport : e === 2 ? this.wrapperElement : window;
    }
    getViewportRange() {
      let { viewport: e, mountPoint: n, stickyTop: t = 0 } = this.options, i = this.getViewportType(), m = 0, h = 0;
      if (i === 0) {
        let s = this.getViewportRelativeTop(), c = Math.max(t, s);
        m = -s + c, h = window.innerHeight - c;
      } else {
        i === 2 && (e = this.wrapperElement);
        let s = e.getBoundingClientRect();
        m = e.scrollTop, h = s.bottom - s.top;
      }
      return m *= this.scrollPosMultiplier, { top: m, bottom: m + h };
    }
    getViewportRelativeTop() {
      let e = this.gridSizingElement.getBoundingClientRect();
      if (this.getViewportType() === 0) return e.top;
      let t = this.getViewportElement().getBoundingClientRect();
      return e.top - t.top;
    }
  ...
  function ReactGRi4D(props) {
    const mountPointRef = (0, import_react.useRef)(null);
    const gridRef = (0, import_react.useRef)(null);
    (0, import_react.useEffect)(() => {
      if (!mountPointRef.current) return;
      const mountPoint = props.viewport instanceof HTMLElement ? props.viewport : mountPointRef.current;
      gridRef.current = new M({
        ...props,
        mountPoint
      });
      return () => {
    ...
    return /* @__PURE__ */ (0, import_jsx_runtime.jsx)("div", { className: "ReactGRi4D", ref: mountPointRef });
  }
})();
